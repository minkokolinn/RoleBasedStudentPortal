...
if ($_SESSION['adminlogin'] == 1) {
	...
	echo "<input type='hidden' class='form-control' value='$adname'  readonly>";
	echo "<input type='hidden' name='adminid' value='$adminid'>";
}else{
	echo "<script>alert('You haven't login as admin')</script>";
	echo "<script>location='ideaShowcase.php'</script>";
}
...
if ($condition == "exit") {
	...
	if (isset($_GET['did'])) {
		...
		$sql = "INSERT INTO rating_info (ideaid,adminId,rating_action) values (:ideaid,:adminid,:rating_action)";
		$insertstmt = $conn->prepare($sql);
		$insertstmt->bindParam(":ideaid", $ideaid);
		$insertstmt->bindParam(":adminid", $adminid);
		$insertstmt->bindParam(":rating_action", $didr);
...
if (isset($_POST['btnSubmit'])) {
	...
	$sql = "INSERT INTO comment (comment,ideaId,adminId) values (:comment,:ideaid,:adminid)";
	$insertstmt = $conn->prepare($sql);
